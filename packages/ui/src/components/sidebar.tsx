...
function SidebarProvider({
  ...
  // Helper to toggle the sidebar. Desktop collapsing is disabled.
...
function Sidebar({
  ...
  /*
   * The caller's DOM props land on `SheetContent`, the element that holds the children — the same
   * place they land on desktop (the container below). `Sheet` is Radix `Dialog.Root`, which renders
   * no element, so props spread onto it were silently discarded: an `onClick` delegated from the
   * sidebar worked on desktop and never fired on a phone, with no error anywhere.
   */
  if (isMobile) {
    return (
...
const sidebarMenuButtonVariants = cva(
  'ring-sidebar-ring hover:bg-sidebar-accent hover:text-sidebar-accent-foreground active:bg-sidebar-accent active:text-sidebar-accent-foreground data-[active=true]:bg-sidebar-accent data-[active=true]:text-sidebar-accent-foreground data-[open=true]:hover:bg-sidebar-accent data-[open=true]:hover:text-sidebar-accent-foreground gap-2 rounded-md p-2 text-left text-sm transition-[width,height,padding,background-color,color] duration-200 ease-out group-has-data-[sidebar=menu-action]/menu-item:pr-8 group-data-[collapsible=icon]:size-8! group-data-[collapsible=icon]:p-2! focus-visible:ring-2 data-[active=true]:font-medium peer/menu-button group/menu-button flex w-full items-center overflow-hidden outline-hidden disabled:pointer-events-none disabled:opacity-50 aria-disabled:pointer-events-none aria-disabled:opacity-50 [&_svg]:size-4 [&_svg]:shrink-0 [&_svg]:transition-transform [&_svg]:duration-200 [&_svg]:ease-out [&>span:last-child]:truncate',
  {
    variants: {
      variant: {
