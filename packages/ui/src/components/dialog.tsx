  ...
  return (
    <DialogPortal data-slot="dialog-portal">
      {/* No hard-coded z-index — Tailwind `z-50` on both overlay and content
          keeps dialogs above the page, and DOM mount order naturally stacks
          multiple open dialogs (each new portal mounts after the previous,
...
function DialogTitle({ className, ...props }: React.ComponentProps<typeof DialogPrimitive.Title>) {
  return (
    <DialogPrimitive.Title
      ...
      className={cn(
        'text-lg leading-none flex items-center justify-between font-semibold',
        className,
      )}
      {...props}
    />
...
function DialogDescription({
  ...
  return (
    <DialogPrimitive.Description
      ...
      className={cn('text-muted-foreground text-sm flex items-center justify-start', className)}
      {...props}
    />
  );
}
