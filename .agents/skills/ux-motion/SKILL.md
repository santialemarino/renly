...
- **Two dropdown families — one animates on close, one doesn't; know which you're picking.** Both use `data-[state=open/closed]` animation classes, but only one gets to run them:
  - **Popover-based comboboxes** — `FilterCombobox` (toolbar single-select filters, e.g. the expenses category/payment filters), `ComboboxMultiSelect`, `CurrencyCombobox` — are Radix `Popover` + `Command`/cmdk. Popover **defers unmount** via `Presence`, so they animate **open AND closed** (the smooth fade/zoom-out you see on the toolbar filters).
  - **Radix `Select`** (`Select`/`SelectTrigger`/`SelectContent` — the form-field dropdown used for income/expense/feedback category fields) animates **open only**. Its content unmounts **instantly** on close (verified: the content element is gone within a frame, whereas Popover/Dialog linger in `data-[state=closed]` for the animation then unmount), so the `data-[state=closed]:animate-out fade-out-0 zoom-out-95` classes shadcn ships on `SelectContent` never run — it snaps shut. This is the primitive's behavior, identical for every Radix `Select` call site, **not** a per-call-site bug: never wrap one `Select` in `AnimatePresence` or fork it to "fix" a single dropdown.
  - **So:** need a dropdown that animates closed → use the Popover-based combobox, not Radix `Select`. Form category fields currently use Radix `Select` (accepting the instant close for the native-select feel); giving them a closing animation is an app-wide decision — switch those fields to the Popover combobox pattern, or fix the shared `@repo/ui` `Select` (a deliberate change to a third-party shadcn component). The trigger chevron rotate on open (`data-[state=open]:…rotate-180`) is animated in both families. For any new dropdown/overlay, verify the close actually plays rather than assuming the `data-[state=closed]` classes imply it.
