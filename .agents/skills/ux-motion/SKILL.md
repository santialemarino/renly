...
- **`Select` is the exception — it animates open but NOT closed.** Radix's `Select.Content` unmounts the moment it closes (it does not defer via `Presence` the way dialog/popover/tooltip/sheet do), so the `data-[state=closed]:animate-out` classes shadcn ships on it never get a frame to run — every select in the app snaps shut. This is a shared-primitive limitation, identical for every `Select` call site, **not** a per-call-site bug: don't add wrappers, `AnimatePresence`, or a forked select to "fix" one dropdown. If a closing animation is ever actually wanted, change the one shared `@repo/ui` `Select` (a deliberate, app-wide call — it's a third-party shadcn component, so treat it like any other shared-component change). The trigger's chevron rotating on open (`data-[state=open]:…rotate-180`) IS animated and unaffected. The same "does the primitive defer unmount?" question applies to any new dropdown/overlay: verify the close actually plays rather than assuming the `data-[state=closed]` classes imply it.
