...
- **inline text links** — links rendered as text (auth links, footer/legal links, a wordmark-as-home link), not buttons: use ONE shared inline-link component — the `AuthLink` pattern (`apps/web/app/(auth)/_components/auth-link.tsx`), generalized with `color`/`size`/`className` props and promoted to a shared location once a second surface uses it. It MUST: animate the underline on hover (`underline decoration-transparent underline-offset-2 … hover:decoration-<color>` + `transition-colors`), give a focus-visible cue **distinct from hover** (`focus-visible:animate-focus-bump-subtle`), sit `inline-block` (so the bump never shifts surrounding text), and drop the native outline (`outline-none`). Never a bare `<a>` with ad-hoc classes.
