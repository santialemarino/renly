...
import { hasSharedSpending } from '@/app/(protected)/shared/shared-expense-rules';
...
export default async function GroupHubPage({ params }: GroupHubPageProps) {
   ...
   *
   * The three money reads are the flow half: what has been spent, where everyone stands, and what
   * has cleared a balance. None of them is caught, deliberately — an empty list rendered from a
   * failed read would say "nothing shared yet" about a group that has shared plenty, which is worse
   * than the page failing. That is the posture `getPots` already takes on this page.
   *
   ...
   */
  const [pots, expenses, balances, settlements, moneySettings, accounts, supportedCurrencies] =
    await Promise.all([
      getPots(group.id),
  ...
  return (
    <div className="flex flex-col flex-1 p-8 gap-y-6">
      ...
      {/*
       * The money block, in the order D29 sets: where everyone stands, what was spent, and what has
       * already cleared. Balances lead because they are the question a household opens the page with.
       */}
      {balances && (
        <GroupBalancesSection
          group={group}
          balances={balances}
          hasSharedSpending={hasSharedSpending(groupExpenses)}
