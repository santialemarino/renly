...
# Counts the payment obligations naming this account as their default funding account. Backs the currency lock:
...
async def count_by_default_account(session: AsyncSession, account_id: int, user_id: int) -> int:
    result = await session.execute(
        select(func.count()).where(
            PaymentObligation.default_account_id == account_id,
            PaymentObligation.user_id == user_id,
        )
    )
