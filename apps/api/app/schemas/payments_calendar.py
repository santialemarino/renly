...
class PaymentsCalendarItemResponse(BaseModel):
    ...
    date: date_type = Field(description="Event date (installment / billing / due / card_due).")
    ...
    cuota_index: int | None = Field(default=None, description="1-based installment index (installment plans only).")
    installments_count: int | None = Field(default=None, description="Total number of installments (installment plans only).")
