...
# unique, which is what makes re-subscribing an upsert rather than a duplicate — and what makes handing
# the browser to a DIFFERENT account a privileged action rather than one, since the owner-match policy
# refuses a conflict-update it cannot see (push_subscription_repository.release_endpoint).
#
...
class PushSubscription(SQLModel, table=True):
    ...
    endpoint: str = Field(unique=True, description="The push service URL for this browser; unique, so re-subscribing replaces its own row.")
