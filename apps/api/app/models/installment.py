...
# Installment plan (a.k.a. "cuotas" locally; e.g. TV Samsung 12x). Auto-generates one
# expense per installment each month (Step 3).
class Installment(SQLModel, table=True):
    ...
    total_amount: Decimal = Field(max_digits=18, decimal_places=2, description="Total purchase amount across all installments.")
    installment_amount: Decimal = Field(max_digits=18, decimal_places=2, description="Amount of each installment.")
    ...
    current_installment: int = Field(default=1, description="Index of the next installment to issue (1-based).")
    ...
    default_account_id: int | None = Field(
        ...
        description="Optional account the scheduler links each emitted cuota to (non-card payment methods only).",
    )
