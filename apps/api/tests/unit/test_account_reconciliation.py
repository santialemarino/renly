...
def _wire(monkeypatch, account: Account, *, income=None, expenses=None, settlements=None, today: date = TODAY) -> dict:
    captured: dict = {}
...
class TestComputeBalanceAt:
    ...
    async def test_sums_are_bounded_by_the_as_of_date(self, monkeypatch):
        ...
        assert income.await_args.kwargs["as_of_date"] == date(2026, 6, 30)
...
class TestCreateReconciliation:
    ...
    async def test_re_reconciling_the_same_date_is_self_correcting(self, monkeypatch):
        ...
        captured = _wire(monkeypatch, _account(), income={7: Decimal("250")})
...
class TestReadAndDelete:
    ...
    async def test_delete_commits_once(self, monkeypatch):
        monkeypatch.setattr(svc, "get_reconciliation", AsyncMock(return_value=object()))
