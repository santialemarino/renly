...
| `409` | Conflict -- e.g., trying to change an investment's currency when it already has snapshots, changing an account's currency or opening date once entries link to it, editing or deleting an expense/income row that a reconciliation owns (`reconciliation_owned_entry`), a change that would leave a group with no admin (`group_last_admin`), a second seat claimed in one group (`group_membership_exists`), an invite to a seat already claimed (`group_seat_taken`), or a partial UNIQUE INDEX rejection on the expense entries table (manual entry duplicating a scheduler-emitted row for the same subscription / installment on the same date). |
