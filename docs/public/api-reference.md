...
Reconciling is repeatable, and works forward. An account holds **one reconciliation per date**: re-running a date you have already reconciled **replaces** it — the earlier row and its adjustment are dropped, and the figures are recomputed against the untouched ledger — so correcting a balance you mistyped is just entering it again. Reconciling a date **earlier** than the account's latest reconciliation is still rejected (400 `account_reconciliation_before_last`): its adjustment would post underneath the newer reconciliation, which is bounded to its own date and cannot see it, leaving that newer balance wrong. To revise an older date, delete the newer reconciliation first.
...
| `POST`   | `/accounts/{id}/reconciliations`       | Reconcile the account against its real balance; replaces any reconciliation already on that date.                                                          |
...
**Computed-balance response fields:** `balance` (the derived balance at `as_of_date`), `bearers` (who a difference would divide between on a shared account — see below) and `replaces_existing` (whether the account already has a reconciliation on that date, which saving would replace). `balance` **excludes** the adjustment of that existing reconciliation, because saving drops it — so the difference the preview shows is the one that will actually be posted.

**Reconciliation request fields:** `as_of_date` (the date the real balance was read — today or earlier, not before the account's `opening_date`, and not before the account's most recent reconciliation; all three rejected with 400) and `statement_balance` (Decimal; may be negative for an overdraft). Posting on a date the account already carries returns **201** with the fresh row, the same as a first reconciliation — the replaced row and its adjustment are gone.

**Reconciliation fields:** `as_of_date`, `statement_balance`, `computed_balance` (what Renly derived at that date, recorded at the time), `difference` (`statement_balance - computed_balance`), `adjustment_expense_id` / `adjustment_income_id` (a private account's adjustment, if any), `adjustment_shared_expense_id` / `adjustment_shared_income_id` (a shared account's — see below), `reconciled_by` (who ran it, as the group names them; null on a private account) and `reconciled_at`.

Deleting a reconciliation cascades to its adjustment entry, so the balance returns to what it was before the true-up. Only the account's **most recent** reconciliation can be deleted (400 `account_reconciliation_not_latest` otherwise), because an older one's adjustment is already baked into every later reconciliation's recorded `computed_balance`. Delete newest-first. Since one date holds one reconciliation, the latest date names exactly one deletable row.
...
#### Reconciling a shared account

An account a pot holds reconciles through exactly the same endpoints, with one difference: the adjustment is a **shared expense or shared income** of the pot's group rather than a private entry, split across the pot's **owners in their ownership proportions on that date** — the same figure on both sides of each member's split. That nets to zero between them, which is the honest record: the drift is money the pot already held, so correcting it changes what each owner _holds_ (pro-rata, through the pot's value) and creates no debt between them. The adjustment is funded from, or paid into, the account being reconciled, and carries the same `account_adjustment` category, so it appears in the group's expense or income list and in each owner's own `/expenses` or `/income` as their share.

The adjustment row is read-only for the same reason a private one is: `PUT` and `DELETE` on a shared expense or shared income carrying an `account_reconciliation_id` are refused with **409** `reconciliation_owned_entry`.

**Who may reconcile a shared account:** any member who can **see** the pot — not only one with write access. It is the same gate the equivalent manual act carries, since a shared expense drawn from that account needs only group membership and a visible, divided pot. Every account response carries `can_reconcile` (computed, read-only) so a client never offers what the API would refuse: it is always `true` for a private account, and `true` for a pot's account once that pot has been divided. Reconciling a pot **nobody has divided yet** is rejected with **400** `account_reconciliation_pot_not_divided` — the difference is split across owners, and an undivided pot has none on record. The same code is returned when the pot was divided only _after_ the date being reconciled.

Recording or deleting a shared reconciliation is written to the group's activity trail, and notifies the pot's other members whenever the difference is non-zero. A reconciliation that finds no difference is still recorded — "the account was checked and it matched" — but sends nothing, because nobody's share moved.
...
**Evolution response:** `points[]` with `date`, `investment_value`, `cash_balance`, `card_balance`, `shared_value`, `private_net_worth`, `net_worth` per month, plus `skipped_currencies`. The grid runs from the earliest month ANY term begins — your first snapshot, an account's opening date, a card's first charge, or a pot's ownership baseline — through the current calendar month, clipped to the requested window at both ends. Investments forward-fill onto it; cash, card debt and the shared side are derived at each month end rather than accumulated, so each is converted at that month's rate. `card_balance` is the outstanding bucket restated at each month's rate, which is why — for any window ending at the current month, including no window at all — the last point's card figure matches the overview's `credit_card_balance`. A window you end in the past is the exception, and correctly so: that point describes the month you asked about, while the overview always describes today.
...
**Net worth formula:** `investment_total + cash_total - credit_card_balance`. Cash comes from your accounts (see [Accounts](#accounts)); each account balance and each card bucket is converted to the display currency at today's rate for current figures and at each month-end for the historical series.
...
| `GET`    | `/groups/{id}/activity`                   | The group's recent activity from its audit trail, newest first. Optional `limit` (1–50, default 20).                     |

`kind` is one of `household`, `couple`, `trip`, `flat`, `other`. `role` is `admin` or `member`.

**Activity** is who did what to which shared thing, and when. Each entry reports an `entity_type` (`group`, `group_member`, `group_invite`, `group_money_settings`, `pot`, `ownership_event`, `shared_expense`, `shared_income`, `settlement`), an `action`, the `entity_id` it concerns, the `actor_name` as the group knows them (`null` once that account is deleted), a `payload` of the values its sentence interpolates, and `created_at`. **No sentence is stored** — the app assembles it from its own translations, for the reason the notification feed does. Entries about a pot you cannot see are simply absent, so this list never states more than the pot pages themselves would.
...
| `GET`    | `/pots/{id}/holdings/contributable`  | Your own private holdings that could be **contributed** to this pot right now, each with its value. Needs pot write access.                                                 |
| `POST`   | `/pots/{id}/holdings/contribute`     | Contribute **one** of them: it is valued, you are issued units worth exactly that, and only then does it move. Body: `investment_id` **or** `account_id`, optional `notes`. |
...
Holdings may move into or out of a pot **as a plain move** only while its ownership has not been agreed yet, and the two directions are refused for opposite reasons. Taking one OUT (`409 pot_already_divided`) would drop the pot's value by the whole of it while nobody's units change — every co-owner's share falling pro-rata so one person's private scope can gain it, with no cap on the amount; the supported act is a withdrawal or a buy-out, both of which redeem units. Putting one IN (`409 pot_holding_add_divided`) is the mirror: the value RISES with nobody's units changing, so what came wholly out of one person's private scope is handed to every owner pro-rata — a gift, made silently. Before the baseline there is nothing to take from or give to anyone, which is what keeps undoing a mistaken move-in possible.

**Adding is still available afterwards, priced** — that is what `POST /pots/{id}/holdings/contribute` is. It values the holding as it stands, issues your seat units worth exactly that at the pot's current unit price, and only then moves it, so your new share is worth what you put in and **nobody else's share loses a cent**; what changes for them is their percentage, because the whole is now bigger. Three things it deliberately does not take. No **amount** — the value is a fact about the holding, and a figure typed higher than it would issue units against value that never arrived. No **date** — a holding counts in the pot's value from the moment it moves, so pricing it at an earlier one would issue units for what it was worth then against an asset the pot gains at what it is worth now, and hand the difference out pro-rata. And no **member** — the holding is yours, so the share is.

It refuses seven things, and `GET /pots/{id}/holdings/contributable` applies all seven as filters, so every row it offers is a row the write accepts. A holding that is not your own private one, or that is already in a pot, is a `404`. An **archived** one is `400 pot_holding_inactive`: it contributes nothing to the pot's value, so units issued against it would dilute every other owner for nothing. One with no valuation on record is `400 pot_holding_unvalued`, and one worth zero or less — including a figure that rounds away to nothing once converted into the pot's currency, or that buys less than a millionth of a unit at this pot's price — is `400 pot_holding_not_positive`. A currency with no stored rate is `503 exchange_rate_unavailable`. And an **account with linked entries** is refused exactly as a plain move refuses it. The pot itself must also be divided and valuable: with no units outstanding it is `400 pot_not_opened` (use the plain move instead) and with no known value today `400 pot_valuation_required`.
...
**A re-agreement can be AGREED TO, and agreeing locks it.** It changes no figure: the entry counted from the moment it was recorded, so a "pending" state would have left the pot showing percentages everybody agrees are wrong. What agreement changes is who may undo it — while unconfirmed either named member may delete it, and once agreed **nobody** can, the pot's own writer included, until the member who agreed withdraws that agreement. Same shape as a settlement's payee-confirm, and for the same reason: it is the trust anchor rather than a permission.

**Who may agree is the affected member, and one rule decides it**: the member LOSING units, unless they recorded the change themselves, in which case it is the member receiving them (`403 pot_reagreement_not_yours` for anybody else, including a pot writer). Stated that way rather than as "either named member", because a third party with write access can record a change between two other people — and then the member who gained units must not be able to lock the member who lost them out of their remedy. A **name-only** member cannot agree, so a re-agreement whose affected member is one is simply never confirmable and stays deletable instead: the same posture a settlement takes. Only a re-agreement carries an agreement at all (`409 pot_event_not_confirmable`), and an entry already agreed to is refused both a second agreement and a deletion (`409 pot_reagreement_confirmed`). Every response on the ledger carries `confirmed_at` plus `can_confirm`, `can_unconfirm` and `can_delete`, already resolved for the caller. Both directions are announced to whoever can see the pot and recorded in the group's activity trail — withdrawing an agreement re-arms a deletion the lock had closed off, which is a real change to the other member's standing.
