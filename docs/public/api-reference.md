...
**Card fields:** `name`, `closing_day` (1-31), `due_day` (1-31), `currency` (ISO 4217 — the card's primary/statement currency), `is_active` (boolean), `monthly_payment` (optional Decimal ≥ 0 — typical monthly payment for revolving-debt users; counts as a fixed commitment on the dashboard Liquidity card when set; null = pay-in-full, excluded from the ratio), `default_account_id` (optional — the cash/bank account this card is normally paid from, i.e. _débito automático_; must be denominated in the card's own currency (400 `account_currency_mismatch`) and owned by you (404 `not_found`); send `null` to clear), `has_expenses` (computed, read-only), `balances` (computed — list of `{currency, balance}` per currency with activity; primary always present, others added by expense activity in non-primary currencies; balances are NOT converted across currencies).
