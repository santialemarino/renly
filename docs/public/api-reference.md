...
**Who may delete a ledger entry is write access — with one exception.** Either member a **re-agreement names** may always delete it, with or without write access to the pot. That is not a convenience: write access is granted to the pot's creator and to nobody else, so out of the box the creator can move units away from a co-owner who is told so by name and would otherwise have no way to undo it. It is narrow in three ways — only a re-agreement (a contribution or a withdrawal moves the mover's own money, and an opening is the division everyone agreed to), only the two seats it names, and only deletion: nobody gains the ability to record anything. **Every deletion is announced** to whoever can see the pot, because undoing an act changes what people own as much as making it did.
