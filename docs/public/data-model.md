...
That is why card reconciliation has no forward-only rule while the account version does. Both sides _replace_ a re-run of the same slot, but only a card period is a window you can re-run: reconciling the flagged statements oldest-first recomputes each from scratch and converges on the correct figures, so out-of-order work is allowed and staleness is the signal rather than a refusal. An account reconciliation is a point-in-time figure with no window to re-run, which is why an older date is refused there. A period that has not closed yet cannot be reconciled at all.
...
The balance is **derived at query time**, never stored -- the same principle as credit-card balances: `opening_balance + linked income − linked expenses − settlements paid from the account ± both transfer legs ± both pot-ownership legs − shared expenses drawn from the account + shared income paid into it ± both settle-up legs`. Expenses, income, and settlements each carry an optional `account_id` (a NULL link is unattributed and affects no balance). Balances may be negative (a real overdraft). An account's currency is fixed once entries link to it, so the balance never mixes currencies.
...
It is deliberately simpler than the card version. An account is single-currency and its balance is a point-in-time figure, so there is no statement **period**, no currency bucket and no `stale` flag — just `as_of_date`, `statement_balance`, the `computed_balance` at that date, and their `difference`.

An account holds **one reconciliation per date**. Re-running a date you have already reconciled replaces it: the earlier row and its adjustment are dropped, and the figures are recomputed against the untouched ledger — so a mistyped balance is corrected by entering it again, and the reconcile dialog says so before you save. The uniqueness is a database constraint rather than a rule the app applies, because the two-row state it prevents was silently destructive: a delete guard that compares dates called both rows the latest, so deleting the older one took its adjustment out of the ledger while the survivor's recorded `computed_balance` still counted it.

Reconciliation is nonetheless forward-only: a date **earlier** than the account's most recent reconciliation is rejected, and only the most recent reconciliation can be deleted. Both guards exist for the same reason — a reconciliation's `computed_balance` is bounded to its own date, so it cannot see an adjustment inserted behind it, and it would silently stop matching the balance the user attested to. Revising an older date means deleting the newer reconciliation first.
...
**A shared account reconciles the same way, and its adjustment lives one table over.** An account a pot holds has no single owner, so the adjustment cannot be a private expense or income row: those belong to exactly one person, and the shared account's balance is derived from movements its own scope owns, so such a row would leave the drift exactly where it was. It is a **shared expense or shared income** of the pot's group instead, funded from — or paid into — the account being reconciled, and split across the pot's **owners in their ownership proportions on that date**. Each owner's split carries the same figure on both sides (what they bore and what they fronted), so the split nets to zero between them: the drift is money the pot already held, so correcting it changes what each owner _holds_, through the pot's value, rather than creating a debt between them. The reconciliation row records which kind of adjustment it posted, and which one it may hold is a database rule rather than a convention — a pot's reconciliation can only carry a shared adjustment and a private one only a private adjustment.

Who may run one is **anyone who can see the pot**, not only a member with write access: it is the same gate the equivalent manual act carries, since a shared expense drawn from that account needs only group membership and a visible, divided pot. A pot **nobody has divided yet** cannot be reconciled at all — the difference is split across owners and there are none on record — and the accounts list says so per row rather than letting the action fail. Recording or deleting one is written to the group's activity trail, and tells the pot's other members whenever the difference is non-zero; a reconciliation that matches is recorded and announces nothing, because nobody's share moved. A **replace** writes both entries — the removal of the row it supersedes and the new one — so the trail cannot be read as two corrections that both stuck, but sends a single message, because the member performed a single save. That message describes what moved: the new difference when it is non-zero; when the new one matches, the superseded correction coming back out of everyone's share, at its amount; and nothing at all when both were zero, since then no share moved.
...
**A group is exported and cannot be restored, and the asymmetry is the point.** Everything shared — the group, its roster and invitations, its money settings, its pots with their access and ownership ledger, its shared expenses, income, splits, settlements and activity trail — is in your data export, because a group you belong to is part of what Renly holds about you. None of it comes back through a restore, because a restore holds one member's file and can only speak for that member: rebuilding the group from it would stand everybody else up as a placeholder bearing a real person's name, owning shares and owing money they never agreed to. Restoring the group alone is no better — seeing a group IS being a member of it, so a group restored without its roster would be invisible to the person who restored it. The restore preview names every one of these sections rather than dropping them in silence.
...
A holding can only be contributed while there is a value to price it against — its own, and the pot's. Something nobody has valued has nothing to issue units for; something archived is not counted in the pot at all, so units against it would dilute everyone for nothing; and something worth zero or less is not a contribution. An account with expenses, income or transfers on it cannot be shared by any route, priced or not, because its balance derives from one person's rows and would read differently for each member.
