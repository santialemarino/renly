...
**Default funding account.** A card can optionally name the account it is normally paid from (Argentine _débito automático_). It only **pre-fills** the "Paid from" field when you record a payment — Renly never creates a settlement from it, because a real auto-debit can fail and inventing a payment that did not happen would leave you deleting a phantom one. The account must be in the card's own currency, since a settlement's account has to match the bucket being settled. Deleting the account clears the default rather than blocking the delete: a default is a convenience, not a record of money that moved.
