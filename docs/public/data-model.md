...
On the Payments Calendar, paid cycles (any obligation cycle whose period contains a linked expense) show with a "Paid" badge instead of the default obligation badge. The calendar walks BOTH forward and backward from the current `next_due_date` so the user sees both unpaid future events AND past paid events inside the viewed month.
