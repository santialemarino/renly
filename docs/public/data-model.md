...
`difference = statement_balance − computed_balance`. A positive difference (the account holds more than the app knew) creates an **income**; a negative one creates an **expense**; zero creates nothing. The adjustment is dated on `as_of_date`, linked to the account so it enters the running balance from there forward, tagged `source = 'reconciliation'`, and pointed back at its reconciliation via `account_reconciliation_id`. One dedicated enum value on each side keeps true-ups out of real spending breakdowns: `expense_category.account_adjustment` and `income_category.account_adjustment`.
