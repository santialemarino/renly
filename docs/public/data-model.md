...
The `source` field indicates how the entry was created: `manual` (entered by the user), `auto` (generated by the system), or `shortcut` (from the iOS Shortcut). The `investment_returns` category is auto-generated monthly by the investment bridge job -- it computes each investment's monthly return (value change adjusted for deposits/withdrawals) and creates income entries so investment gains appear in the financial dashboard.
