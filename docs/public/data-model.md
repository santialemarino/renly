...
Ownership is the foreign key, not the `source` value. `source` records provenance and survives a restore, which nulls both links because the reconciliation tables are not restored — so a restored adjustment is an ordinary historical entry that nothing owns, and it stays editable and deletable. Its system-generated category is still not offered in the picker, so the entry form cannot re-save it; the Edit action stays hidden for that separate reason while Delete remains available.
