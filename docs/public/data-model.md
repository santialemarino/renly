...
An expense entry records money going out. Each entry has a date, amount, currency, optional category, payment method, notes, and a source indicating how it was created (`manual`, `shortcut`, `auto`, `email_parsed`, `subscription`, or `installment`).

When the source is `subscription` or `installment`, the entry was auto-generated by the daily scheduler and points back to the source plan via `subscription_id` or `installment_id` (both nullable; deleting the plan keeps the historical expense and clears the link). The pair `(source plan, date)` is unique, so re-running the scheduler is a no-op.
