...
The two reminders share a waking morning hour and the auto-expense job's is not, deliberately: a reminder is a message asking somebody to do something, where the auto-expense job is a silent background write nobody reads.

**The digest's hour is later than both reminders', and that ordering is load-bearing.** All four jobs filter on the SAME person's local clock, so a reminder raised at that person's 09:00 is always already queued when their 20:00 summary is built — a relationship that holds in every timezone and depends on no job registration order. `tests/unit/test_scheduler_config.py` asserts it as an inequality between the three constants.

`obligation_reminder_service.py` carries one more schedule parameter, beside its hour:
...
### 7. Obligation reminders

| Property         | Value                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| ---------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Trigger**      | `cron` (hourly, minute 0)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| **Service call** | `obligation_reminder_service.send_due_reminders(session)`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| **What it does** | Tells each owner about payment obligations falling due within `OBLIGATION_REMINDER_LEAD_DAYS` (3). Hourly because the filter is per user: only owners whose local time is now `OBLIGATION_REMINDER_HOUR_LOCAL` (9) are considered. A payment obligation is the one scheduled payment in Renly nothing charges automatically — a subscription and an installment both emit their own expense (job 4) — so it is the only one where a heads-up is both true and actionable. Recurring and one-off obligations are both announced; the window has no lower bound, so a bill already overdue is announced once too. |
| **Idempotence**  | Through the notification's `dedupe_key` (`obligation:<id>:<due date>`) and the partial unique index behind it — not through state of its own. A bill sits inside the window for the whole lead period, so the offer is made every day and all but the first are refused. Marking it paid advances `next_due_date`, which is what makes the NEXT cycle a different key rather than a repeat.                                                                                                                                                                                                                     |
| **Cost**         | Prunes in SQL first: `list_active_due` bounds the scan to obligations plausibly due (the lead plus one day, covering every user's local today up to UTC+14), and the per-user local-date comparison in Python is the correctness layer. Owners are grouped before the hour test, so it is asked once per person rather than once per bill.                                                                                                                                                                                                                                                                      |
...
| ---------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Trigger**      | `cron` (hourly, minute 0)                                                                                                                                                                                                                                                                                                                                                                                                          |
| **Service call** | `notification_digest_service.send_due_digests(session)`                                                                                                                                                                                                                                                                                                                                                                            |
| **What it does** | Sends one email summarising everything a person was told since their last summary, to everybody who set their email cadence to `daily`. Hourly, filtered to users whose local time is now `DIGEST_HOUR_LOCAL` (20). Each LINE of the email is that event's own email subject, so the digest needs no per-event copy and a new event is digestible the day it is added. Capped at `DIGEST_MAX_ITEMS` (40) with the remainder named. |
| **Idempotence**  | Through `notifications.digest_pending`, a per-row queue written at dispatch by the same predicate that decides the email is not sent immediately. A per-row flag rather than a per-user cursor, because a cursor has to be initialised when somebody switches cadence and getting that wrong emails them their whole history.                                                                                                      |
| **Cost**         | Prunes before it measures: the FIRST read is the queue itself, which a partial index (`ix_notifications_digest_pending`) answers in one probe — so an hour with nothing owed loads no timezone, no user and no notification.                                                                                                                                                                                                       |

The queue is cleared whether or not the send succeeded, the same posture every other send in this layer takes: the notifications are already in the recipient's feed, so a provider outage must not leave a queue that re-sends the same summary every day until it recovers.

### 9. Auto-snapshot startup catch-up
