...
**Entry forms** (expense / income / subscription / installment / payment obligation) **and the investment form** (`base_currency`) only OFFER the supported set — the full-ISO "Other currencies" group is hidden there, and the API rejects an unsupported currency with **422** (finance entries and `base_currency` share the `validate_supported_currency` field-validator; snapshot/transaction rows are typed by the `Currency` enum). The warning behaviour in §7 applies only to the **display and preference** pickers (which keep the full ISO list): a currency without exchange-rate support can still be selected for display/preferences. Under it, a single ROW falls back to its original amount labelled with its own currency, while a TOTAL leaves every unconvertible row out (reading 0 when none convert) and the page names the skipped currencies — see §7's fallback behaviour.
...
**One rounding rule.** Every money figure in the app rounds **half-up**, through `domain/money.py`'s `quantize`. Half-up rather than Decimal's `ROUND_HALF_EVEN` default (banker's rounding) because these are figures a person reads and checks by hand. Two spellings take banker's rounding silently and are therefore the ways a second rule arrives: `.quantize()` with no `rounding=` (so `convert_value(4.69, ARS→USD)` at 1 USD = 2 ARS would give 2.34 where the rule gives 2.35), and the builtin `round(x, 2)`, which on a Decimal is `ROUND_HALF_EVEN` as well (10 × 0.2345 would store 2.34). A unit-suite guard (`test_one_rounding_rule.py`) walks `app/` and fails both: any `.quantize()` that omits the mode, and any two-argument `round()` to the money scale — the scale derived from `MONEY_PLACES`, with a non-literal `ndigits` failing too because nothing can prove it is not the money scale. A `round()` at another literal scale passes: the six-place rounding of a provider's quoted price and of the IRR solver's rate are not money figures.

The web holds the same rule in `lib/money.ts`. A figure the browser COMPUTES — a snapshot's value from quantity × price, a re-agreement's worth from units × unit price — goes through `moneyProduct`, which multiplies the two decimals exactly and rounds once, half away from zero. `(1 * 1.005).toFixed(2)` is `"1.00"` because it rounds the binary approximation; `moneyProduct(1, 1.005)` is `"1.01"`, the API's answer for the same inputs. A web unit test fails any `.toFixed(2)` outside that file.

**Money is Decimal end to end.** No sum round-trips through `float`. The three card-bucket repository sums return `Decimal` and the service adds them directly; a `float` in that path is harmless at ordinary magnitudes and wrong past roughly 90 trillion, where a cent falls off the end of a float's ~15–16 significant digits — a figure ARS reaches.
...
- **Aggregates** (finance overview / monthly / breakdowns, dashboard overview / evolution / composition, expense & income lists, payments calendar) exclude the row and list its code in an additive `skipped_currencies: string[]` response field.
  - ▸ **Returning the field is half the job; the surface has to RENDER it.** A total that has silently left rows out looks no different from a correct one (two rows worth 1,974.53 read 1,000.00 with one dropped), so every page reading one renders a `WarningHint` naming the skipped codes: `/dashboard`, `/expenses`, `/income`, `/payments-calendar` and `/finance-dashboard` (`/investor-dashboard` names its skipped investments instead — see §8). The two dashboards render it over the **union** of their responses' skip sets, since each computes its own over the rows it reads and a currency can be missing a rate for one and not another. Two web unit tests hold this: `skipped-currencies-contract.test.ts` checks that the web declares the field on at least as many responses as each API schema module sends it on, and maps every one; `skipped-currencies-hint.test.tsx` renders every page that reads the field — the set derived from the source — and checks the warning names every code.
...
| ------------------------------------------------------- | --------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------- |
| Expense / income list display (private and shared rows) | row's `date`                                  | Past records — historical accuracy. A shared row converts at the group row's own date, so a member's share reads the same as any other entry.      |
...
- **A ROW falls back; a TOTAL does not.** A per-row `converted_*` field comes back null and the UI renders the original amount labelled with its own currency. An **aggregate cannot fall back**: there is one number and it can only be in one scale, so every unconvertible row is EXCLUDED and the total reads **0**, with each source currency named in `skipped_currencies` (two rows worth 1,974.53 USD total 0 under an unsupported display currency). This is the fail-loud rule above applied to the display currency.
...
**A shared expense is single-currency by construction, and so is a piece of shared income.** The amount, the splits and the balance bucket are all the same currency, and the account the money left or arrived in must match it (`400 account_currency_mismatch`) — the account sum carries one amount, so a mismatched link would move a foreign figure straight through the balance. Only the **settlement** crosses currencies, which is where the conversion belongs: it is the moment somebody actually agreed a rate.

**Both flows land in the same bucket, and neither is converted to get there.** A member who fronted a dinner in pesos and a member who collected peso rent are owed and owing in the ARS bucket, so the settle-up plan nets them into one payment. The two are read in opposite directions — a shared expense's position is `paid_amount − amount` and shared income's is `amount − received_amount`, because fronting money is a claim on the group while receiving money is a debt to it — and each has its own domain function so neither can be handed the other's columns. Nothing anywhere converts one flow into the other's currency to combine them: a bucket is a bucket.
...
`skipped_currencies`, which the dashboard renders as a warning — a headline quietly
