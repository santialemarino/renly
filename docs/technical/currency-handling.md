...
**One rounding rule.** Every money figure in the app rounds **half-up**, through `domain/money.py`'s `quantize`. Half-up rather than Decimal's `ROUND_HALF_EVEN` default (banker's rounding) because these are figures a person reads and checks by hand. `convert_value` used to call `.quantize()` with no mode and therefore took the default, so a converted amount landing exactly on a half went the opposite way from the same figure anywhere else — `convert_value(4.69, ARS→USD)` at 1 USD = 2 ARS gave 2.34 where the domain rule gives 2.35, and half of all exactly-half results disagreed. A unit-suite guard now walks `app/` and fails any `.quantize()` call that omits the mode, so a second rule cannot arrive by default.

**Money is Decimal end to end.** No sum round-trips through `float`. The three card-bucket repository sums used to return `float` and the service rebuilt a `Decimal` from `str(total)`; harmless at ordinary magnitudes and wrong past roughly 90 trillion, where a cent falls off the end of a float's ~15–16 significant digits — a figure ARS reaches. The sums now return `Decimal` and the service adds them directly.
