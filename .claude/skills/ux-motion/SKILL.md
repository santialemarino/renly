...
### Icon motion (interactive icons)

Every icon that sits in an interactive element uses **`<AnimatedIcon icon={Lucide} />`** (`apps/web/components/animated-icon/`), never a bare Lucide icon with an ad-hoc hover transform. It gives each icon a bespoke, part-aware motion — a bell rings, a cog turns, a check draws in — that reads true to the icon.

- **Trigger + focus parity.** Mark the nearest interactive ancestor `data-animate-icon`; the motion plays while that element is hovered OR keyboard-focused (`:focus-visible`), so focus parity is built in — the icon needs no separate focus cue. Add the attribute once per surface (the sidebar buttons, `RowActionButton`, `InlineLink`, and the landing feature card all do).
- **Where the motion lives.** The vocabulary is shared CSS in `packages/ui/src/styles/index.css` — whole-glyph families (`aicon-spin` / `-ring` / `-pulse` / `-pop` / `-bob` / `-nudge-*`), part poses (`aipart-turn` / `-slide-x`), and the universal `aicon-draw` — all wrapped in `@media (prefers-reduced-motion: no-preference)` so reduced motion shows **nothing**. Rebuild `@repo/ui` after editing it. Only transform / stroke-dashoffset animate (no layout shift); durations are the `--aicon-*` vars (the animation constants as ms).
- **What each icon does** is the registry (`registry.ts`, keyed by the Lucide component, not its aliased displayName): a whole-glyph family, a part-level **bespoke** component (`bespoke.tsx` — a cog turns, an arrow slides out, a check draws while the rest stays), or the universal **draw-itself-in** fallback so no icon is ever left on a generic transform. To add a bespoke motion, add the geometry to `bespoke.tsx` and register it; to retune a mapping, edit `registry.ts`.
- **Moving triggers.** If the interactive element lifts/moves on hover (e.g. a feature card), keep `data-animate-icon` on a **stationary** ancestor, not the moving layer, so the icon's hover box doesn't flicker (same rule as the hover-lift fix).
- **Not this:** a control's open/closed **chevron** (select / combobox / disclosure) is a state affordance, not hover motion — keep the shared rotating `ComboboxChevron` (see Open/close below); don't route it through `AnimatedIcon`.
